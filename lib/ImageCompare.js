...
class ImageCompare extends EventEmitter {
  ...
  start() {
    ...
    fs.watch(self.capturesDir, (eventType, filename) => {
      ...
      if (eventType !== 'rename' && filename.indexOf('.jpg') > -1 && filename.indexOf('~') === -1) {
        fs.access(path.resolve(self.capturesDir, filename), fs.constants.R_OK, (error) => {
          if (!error) {
            ...
            if (self.controlFileName && self.compareFileName) {
              Jimp.read(path.resolve(self.capturesDir, self.controlFileName), (controlFileError, controlFile) => {
                if (controlFileError) {
                  self.emit('error', controlFileError);
                }

                Jimp.read(path.resolve(self.capturesDir, self.compareFileName), (compareFileError, compareFile) => {
                  if (compareFileError) {
